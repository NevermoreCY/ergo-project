...
(define sw-acts                            ; acts to send to SW server
  '(feel look smell listen left right forward pick drops eat build quit))
(define sw-responses (hasheq               ; exog responses for sensing acts
  'feel 'set-energy! 'look 'set-view! 'smell 'set-aroma! 'listen 'set-sound!))

(define self-action (hasheq 'drops 'drop 'feel 'feel 'look 'look 'smell 'smell 'listen 'listen 'left 'left 'right 'right 'forward 'forward 'pick 'pick 'quit 'quit))
...
(let ((chan (make-channel)) (iport (car sw-ports)) (oport (cadr sw-ports)))
  ...
  (define (sw-write act)                   ; send act over TCP and get response
    (and (memq act sw-acts) ; make sure act is in act list
      (let ()    
...
(define-interface 'out
  (let ((ports (open-tcp-client 8234)))
    (lambda (act)
      
      ;(fprintf (cadr ports) "~a id121" act)
