...
(define num-of-acorns 140)         ; how many randomly positioned acorns
(define num-of-walls 0)           ; how many randomly positioned walls
...
(define max-energy 300000)          ; the max energy a squirrel can have 
...
(define dur-pick 2)              ; picking up an acorn
(define dur-drop 1)              ; dropping an acorn
(define dur-eat 60)               ; eating an acorn
(define dur-build 40)             ; building a wall segment
(define dur-feel 1)               ; sensing energy level
(define dur-smell 1)              ; sensing for an acorn or a squirrel
...
;(define (new-seed) 107280424)  ; use this seed for debug

; used seed : 772694594  32508107
...
(define (initialize-world) 
  ...
  (define (ini-acorns)
    (for/list ((i (in-range num-of-acorns)))
              ...
              (make-acorn #t (random 1 (- cols 1)) (random 1 (- rows 1)) ))) ; Don't put acorns on the border which helps the design 
...
(define (act-pick sq time)
  (let ((holding (squirrel-carrying sq))
   ...
   ;         (> (num-squirrels-at xpos ypos) 1)) 'fail
    (if (= (length holding) max-holdings) 'fail   ; modification so suqirrel can pick up acorn even when there are other squirrel nearby                                               
